                  ...
                  <div className="space-y-2.5">
                    {AUTO_CHANNELS.filter(c => PROVIDER_CHANNELS[autoPayment.provider!].includes(c.id)).map(c => {
                      ...
                      return (
                        <div
                          key={c.id}
                          className={`flex items-center justify-between p-3.5 rounded-xl border-2 transition-all ${on ? 'border-emerald-300 bg-emerald-50' : 'border-slate-200 bg-white hover:border-slate-300 hover:bg-slate-50'}`}
                        >
                          <div className="flex items-center gap-3">
