  ...
  return (
    <section className="py-10 sm:py-24 bg-white">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 xl:px-0">
        ...
        <AnimateOnView delay={0.1}>
          <Accordion defaultValue={['item-0']} className="space-y-2 sm:space-y-3">
            {faqs.map((faq, i) => (
              <AccordionItem
                key={i}
                value={`item-${i}`}
                className="bg-white border border-slate-200 rounded-2xl shadow-sm px-4 sm:px-6 overflow-hidden"
              >
                <AccordionTrigger className="font-semibold text-slate-900 text-left hover:no-underline py-3.5 sm:py-5">
                  {faq.q}
                </AccordionTrigger>
                <AccordionContent className="text-slate-500 text-sm leading-relaxed pb-4 sm:pb-5">
                  {faq.a}
                </AccordionContent>
              </AccordionItem>
            ))}
